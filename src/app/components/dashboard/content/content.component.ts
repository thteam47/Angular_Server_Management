...
export class ContentComponent implements OnInit {
  ...
  columnList: string[] = [];
  columnListName: string[] = ['Server Name','Port', 'Password', 'Username', 'Ip', 'Status'];
  columnL = ['serverName','port', 'password', 'username', 'ip', 'status'];
  ...
  openEdit(element: any): void {
    const dialogRef = this.dialog.open(InfoserverComponent, {
      width: '500px',
      data: { idServer: element.idServer, serverName: element.serverName,port: element.port, username: element.username, password: element.password, ip: element.ip, description: element.description }
    });
  ...
  changePass(element: any): void {
    const dialogRef = this.dialog.open(ChangepassComponent, {
      width: '500px',
      data: { idServer: element.idServer, servername: element.servername, port: element.port, password: element.password, ip: element.ip, description: element.description }
    });
    dialogRef.afterClosed().subscribe(() => {
      setTimeout(() => {
        this.getList();
